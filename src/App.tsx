...
const DEFAULT_SOCIAL_CRAWLER_SETTINGS: SocialCrawlerSettings = {
  ...
  wechatMaxPages: 2,
  wechatRequestIntervalSeconds: 8,
  crawlSubpages: false,
  crawlMaxDepth: 1,
  crawlMaxPages: 6,
  proxyMode: 'off',
...
function normalizeSocialCrawlerSettings(settings: Partial<SocialCrawlerSettings> | null | undefined): SocialCrawlerSettings {
  return {
    ...
    wechatMaxPages: Math.max(1, Math.min(10, Number(settings?.wechatMaxPages || DEFAULT_SOCIAL_CRAWLER_SETTINGS.wechatMaxPages))),
    wechatRequestIntervalSeconds: Math.max(2, Math.min(120, Number(settings?.wechatRequestIntervalSeconds || DEFAULT_SOCIAL_CRAWLER_SETTINGS.wechatRequestIntervalSeconds))),
